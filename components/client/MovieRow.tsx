  ...
  return (
    <div className="space-y-2 group relative">
      ...
      <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white px-4 sm:px-12 flex items-center gap-2 group-hover:text-white transition-colors">
        <span>{title}</span>
        <span className="text-xs text-[#54b9c5] font-semibold opacity-0 group-hover:opacity-100 transition-opacity flex items-center cursor-pointer">
          Khám phá tất cả &gt;
        </span>
      </h2>
      ...
      <div className="relative">
        ...
        <div
          ref={rowRef}
          className="flex items-center gap-2 sm:gap-3 overflow-x-auto no-scrollbar px-4 sm:px-12 py-4 scroll-smooth"
        >
          {displayMovies.map((movie, index) => {
            if (isTop10) {
              return (
                <div
                  key={movie.id}
                  className="flex items-center shrink-0 relative group/item"
                >
